...
def makebatch(args):
	...
	filelist = os.listdir('./basis_json_32')
	...
	for filename in tqdm(filelist):
		images = np.array(json.load(open('./basis_json_32/' + filename))['image'])
		dim = images.shape[0]
		nc = images.shape[-1]
		sum_nc += nc
		for c in range(nc):
...
def main():
	...
	parser.add_argument('--savedir',type=str,default='batch_image_cell/',
